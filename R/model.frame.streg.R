## Adapted from survival:::model.frame.survreg and survival:::model.matrix.survreg.
## Functions in the survival package drop strata terms from the returned
## frame/matrix. streg needs the strata terms for prediction.

#' @export
model.frame.streg <- function (formula, ...) {
  dots <- list(...)
  ...
  if (length(nargs) || is.null(formula$model)) {
    ...
    if (indx[1] == 0)
      stop("The streg call is missing a formula!")
    temp <- fcall[c(1, indx)]
...
#' @export
model.matrix.streg <- function(object, data,  ...) {
  if (missing(data) && !is.null(object[["x"]]))
    object[["x"]]
  else {
    ## drop outcome and cluster terms, but not strata terms
